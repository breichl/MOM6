...
!
! !USES:
   use turbulence, only: P,B,Pb, PV
   use turbulence, only: num,nuh,numS
...
!BOC
   ...
   if ( PRESENT(xP) ) then
      do i=0,nlev
         !Production Term is due to gradients of mean current.
         ! Reynolds stress decomposed to num*du/dz+numS*duS/dz,
         ! hence, additional term in P *PLUS* PV (vortex force)
