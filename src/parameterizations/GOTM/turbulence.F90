...
!  of momentum, temperature, salinity
   ...
   REALTYPE, public, dimension(:), allocatable         :: numS!Stokes
...
!  allocate memory
   ...
   allocate(PV(0:nlev),stat=rc)
   if (rc /= 0) stop 'init_turbulence: Error allocating (PV)'
   PV = _ZERO_
   ...
   allocate(numS(0:nlev),stat=rc)
   if (rc /= 0) stop 'init_turbulence: Error allocating (numS)'
   numS = _ZERO_
...
! !USES:
   ...
   interface
      subroutine production(nlev,NN,SS,SSu,SSv,SSUS,SSVS,xP)
        integer,  intent(in)                :: nlev
        ...
        REALTYPE, intent(in)                :: SSVS(0:nlev)
...
!        with seagrass turbulence
         call production(nlev,NN,SS,SSu,SSv,SSUS,SSVS,xP)
      else
!        without
