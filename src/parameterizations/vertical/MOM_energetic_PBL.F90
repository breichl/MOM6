...
! real    :: Hmix_min        !< The minimum mixed layer thickness in m.
  ...
  real    :: vstar_scale_fac !< An overall nondimensional scaling factor for vstar.
                             !! Making this larger increases the diffusivity.
  real    :: vstar_surf_frac !< A coefficient multiplied by ustar for surface value of vstar
                             !! if vstar_mode == 1
  ...
  real :: RH18_cS2           !< MSTAR_S exponent for RH18 in stabilizing limit. Value of 0.4 in RH18.
      ...
      do OBL_IT=1,MAX_OBL_IT ; if (.not. OBL_CONVERGED) then
        ...
        ! First solve for the TKE to PE length scale
          ...
          elseif (CS%MSTAR_MODE.eq.CS%MSTAR_RH18) then
            MSTAR_N = CS%RH18_cN1 * ( 1.0 - ( 1.+CS%RH18_cN2 * &
                 exp( CS%RH18_cN3 * MLD_GUESS * absf(i) / u_star) )**-1.0 )
            MSTAR_S = CS%RH18_cS1 * (bf_stable**2*MLD_GUESS &
                 / ( u_star**5 * absf(i) ) ) **CS%RH18_cS2
...
!                 "is determined dynamically.", units="m", default=0.0)
  ...
  call get_param(param_file, mdl, "RH18_CN2", CS%RH18_cN2,&
  ...
  call get_param(param_file, mdl, "RH18_CS1", CS%RH18_cS1,&
                 ...
                 units="nondim",default=0.2)
  call get_param(param_file, mdl, "RH18_CS2", CS%RH18_cS2,                                 &
